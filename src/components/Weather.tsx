...
export function Weather({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    let alive = true
    const load = () => ha.getForecast(config.weather).then((f) => alive && setForecast(f.slice(0, 5))).catch(console.error)
    load()
    const t = setInterval(load, 30 * 60 * 1000)
    return () => { alive = false; clearInterval(t) }
  }, [ha])

  if (!w) return <div className="card weather"><div className="muted">Wetter: keine Daten ({config.weather})</div></div>

  const a = w.attributes
  return (
    <div className={`card weather ${compact ? 'compact' : ''}`}>
      <div className="weather-now">
        <WeatherIcon condition={w.state} size={96} />
        <div>
      ...
      <div className="forecast">
        {forecast.map((f) => (
          <div key={f.datetime} className="forecast-day">
            <div className="forecast-name">{new Date(f.datetime).toLocaleDateString('de-DE', { weekday: 'long' })}</div>
            <WeatherIcon condition={f.condition} size={40} />
