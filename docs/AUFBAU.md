...
| Live-Untertitel | Jeder Sprachbefehl erscheint groß in der Bildschirmmitte: gehörter Satz + gestreamte Antwort (Wort für Wort, Cursor), danach Lesezeit-Haltedauer. Quelle: eigenes Mikro (Pipeline-Events) **oder** Handy-App/Satellit – dafür fragt `src/ha/assistRuns.ts` die HA-Debug-Schnittstelle `assist_pipeline/pipeline_debug/list|get` ab (1×/s im Leerlauf, 350 ms während eines Laufs). Reine Wake-Word-Läufe werden ignoriert; ein Mikrofon-Fehler des Browsers bleibt klein am Button |
| Kalender (2. Account) | Zweiter iCloud-Account (vt92@gmx.de) via HA-CalDAV seit 2026-09-03: `calendar.kalender` (Geburtstage), `calendar.arbeit`, `calendar.privat`, `calendar.familie` – alle **pink**. Die Listen Schule/Musik/Verabredungen/… dieses Accounts sind Apple-Erinnerungen (`todo.*`), keine Kalender. Neue Kalender in HA: CalDAV-Eintrag neu laden, in `src/config.ts` eintragen, für Assist freigeben (`homeassistant/expose_entity`) und in `automations.yaml` (kalender_live_refresh) ergänzen |
| Kalender | Apple/iCloud über die HA-Integration **CalDAV** → `calendar.*`-Entities; Termine anlegen per `calendar.create_event`. **Live-Update:** HA-Automation `Kalender live` (`assistant/homeassistant/automations.yaml`) ruft jede Minute, 3 s nach jedem `calendar.*`/`rest_command.*`-Aufruf und 3 s nach **jeder Sprachassistent-Anfrage** (State-Trigger auf `conversation.*`) `homeassistant.update_entity` auf (statt CalDAV-Standard 15 min) und feuert `smarthomeheart_calendar_refreshed`; das Dashboard hört darauf (plus `state_changed`, `call_service`, Tab sichtbar, 60-s-Poll) – Änderungen erscheinen ohne Reload |
