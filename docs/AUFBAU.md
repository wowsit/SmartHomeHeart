...
| Kalender | Apple/iCloud über die HA-Integration **CalDAV** → `calendar.*`-Entities; Termine anlegen per `calendar.create_event`. **Live-Update:** HA-Automation `Kalender live` (`assistant/homeassistant/automations.yaml`) ruft jede Minute und 3 s nach jedem `calendar.*`/`rest_command.*`-Aufruf `homeassistant.update_entity` auf (statt CalDAV-Standard 15 min) und feuert `smarthomeheart_calendar_refreshed`; das Dashboard hört darauf (plus `state_changed`, `call_service`, Tab sichtbar, 60-s-Poll) – Änderungen erscheinen ohne Reload |
